...
test.describe('Suite 5', () => {
  ...
  test.beforeAll(async ({ restage }) => {
    await prepareTestContext(restage, 'suite5');
    _restage = restage;
  ...
  test('Init RML', async () => {
    ...
    await _rmlTest.addAssertionsPath();
    await _asserts.validateAddPath();
    await _rmlTest.updatePathUsername();
    await _asserts.validatePathUsername();
    await _rmlTest.addIsEqual();
    await _asserts.validateAddIsEqual();
    await _rmlTest.addIsTrue();
    await _asserts.validateAddIsTrue();
    await _rmlTest.addIsFalse();
    await _asserts.validateAddIsFalse();
    await _rmlTest.addIsNull();
    await _asserts.validateAddIsNull();
    await _rmlTest.addNotCondition();
    await _asserts.validateAddNotCondition();
    await _rmlTest.addAllMatch();
    await _asserts.validateAddAllMatch();
    await _rmlTest.addAnyMatch();
    await _asserts.validateAddAnyMatch();
  });
